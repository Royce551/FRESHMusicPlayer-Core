...
namespace FRESHMusicPlayer
{
    ...
    public class OldLibrary
    {
        ...
        public OldLibrary(LiteDatabase library)
        {
            Database = library;
        }
        /// <summary>
        ...
        public List<OldDatabaseTrack> Read(string filter = "Title") => Database.GetCollection<OldDatabaseTrack>("tracks").Query().OrderBy(filter).ToList();
        /// <summary>
        ...
        public List<OldDatabaseTrack> ReadTracksForArtist(string artist) => Database.GetCollection<OldDatabaseTrack>("tracks").Query().Where(x => x.Artist == artist).OrderBy("Title").ToList();
        /// <summary>
        ...
        public List<OldDatabaseTrack> ReadTracksForAlbum(string album) => Database.GetCollection<OldDatabaseTrack>("tracks").Query().Where(x => x.Album == album).OrderBy("TrackNumber").ToList();
        /// <summary>
        ...
        public List<OldDatabaseTrack> ReadTracksForPlaylist(string playlist)
        {
            var x = Database.GetCollection<OldDatabasePlaylist>("playlists").FindOne(y => y.Name == playlist);
            var z = new List<OldDatabaseTrack>();
        ...
        {
            var x = Database.GetCollection<OldDatabasePlaylist>("playlists").FindOne(y => y.Name == playlist);
            if (Database.GetCollection<OldDatabasePlaylist>("playlists").FindOne(y => y.Name == playlist) is null)
            {
            ...
            {
                x.Tracks.Add(path);
                Database.GetCollection<OldDatabasePlaylist>("playlists").Update(x);
            }
        }
        /// <summary>
        ...
        public virtual OldDatabasePlaylist CreatePlaylist(string playlist, string path = null)
        {
            var newplaylist = new OldDatabasePlaylist
            {
                Name = playlist,
                Tracks = new List<string>()
            };
            if (Database.GetCollection<OldDatabasePlaylist>("playlists").Count() == 0) newplaylist.DatabasePlaylistID = 0;
            else newplaylist.DatabasePlaylistID = Database.GetCollection<OldDatabasePlaylist>("playlists").Query().ToList().Last().DatabasePlaylistID + 1;
            if (path != null) newplaylist.Tracks.Add(path);
            Database.GetCollection<OldDatabasePlaylist>("playlists").Insert(newplaylist);
            return newplaylist;
        }
        /// <summary>
        /// Deletes a playlist
        ...
        public virtual void DeletePlaylist(string playlist) => Database.GetCollection<OldDatabasePlaylist>("playlists").DeleteMany(x => x.Name == playlist);
        /// <summary>
        ...
        {
            var stufftoinsert = new List<OldDatabaseTrack>();
            int count = 0;
            foreach (string y in tracks)
            {
                var track = new Track(y);
                stufftoinsert.Add(new OldDatabaseTrack { Title = track.Title, Artist = track.Artist, Album = track.Album, Path = track.Path, TrackNumber = track.TrackNumber, Length = track.Duration });
                count++;
            }
            Database.GetCollection<OldDatabaseTrack>("tracks").InsertBulk(stufftoinsert);
        }
        /// <summary>
        /// Imports some tracks to the library
        /// </summary>
        ...
        public virtual void Import(string path)
        {
            ...
            Database.GetCollection<OldDatabaseTrack>("tracks")
                                .Insert(new OldDatabaseTrack { Title = track.Title, Artist = track.Artist, Album = track.Album, Path = track.Path, TrackNumber = track.TrackNumber, Length = track.Duration });
        }
        /// <summary>
        ...
        {
            Database.GetCollection<OldDatabaseTrack>("tracks").DeleteMany(x => x.Path == path);
        }
        /// <summary>
        ...
        {
            Database.GetCollection<OldDatabaseTrack>("tracks").DeleteAll();
            if (nukePlaylists) Database.GetCollection<OldDatabasePlaylist>("playlists").DeleteAll();
        }
        /// <summary>
        ...
        public OldDatabaseTrack GetFallbackTrack(string path)
        {
            var dbTrack = Database.GetCollection<OldDatabaseTrack>("tracks").FindOne(x => path == x.Path);
            ...
            else
            {
                try
                {
                    ...
                    if (track != null) return new OldDatabaseTrack { Artist = string.Join(", ", track.Artists), Title = track.Title, Album = track.Album, Length = track.Length, Path = path, TrackNumber = track.TrackNumber };
                }
                catch
                {
                    // ignored (for now)
                ...
                return new OldDatabaseTrack { Artist = atlTrack.Artist, Title = atlTrack.Title, Album = atlTrack.Album, TrackNumber = atlTrack.TrackNumber, Length = atlTrack.Duration };
            }
        }
    }
    /// <summary>
